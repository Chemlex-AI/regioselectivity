...
def mols_from_smarts_list(all_smarts):
    '''Given a list of smarts strings, this function creates rdkit
    molecules'''
    mols = []
    for smarts in all_smarts:
        if not smarts: continue
        mols.append(Chem.MolFromSmarts(smarts))
    return mols
...
def bond_to_label(bond, consider_atommap=True):
...
def extract_from_reaction_temp(reaction, keep_atom_mapping=False, r_radius=1, p_radius=0, precanonicalize=True):
    reactants = reaction['reactants']
    products = reaction['products']
    if precanonicalize:
        reactants, products = canonicalize_rxn(reactants, products)
    ...
    if None in reactants: return {'reaction': reaction['reactants'] + '>>' + reaction['products'],
                                  'err_msg': 'rdkit cant understand reactants molecule'}
    if None in products: return {'reaction': reaction['reactants'] + '>>' + reaction['products'],
                                 'err_msg': 'rdkit cant understand products molecule'}
    ...
    try:
        ...
        return {'reaction': reaction['reactants'] + '>>' + reaction['products'], 'err_msg': err_msg}
    ...
    if keep_atom_mapping:
        rxn_canonical = '>>'.join([canonicalize_template(x) for x in rxn_string.split('>>')])
...
def extract_from_reaction(reaction, keep_atom_mapping=False, r_radius=1, p_radius=0, acc=10, precanonicalize=True):
    for i in range(acc):
        template = extract_from_reaction_temp(reaction, keep_atom_mapping=keep_atom_mapping, r_radius=r_radius,
                                              p_radius=p_radius, precanonicalize=precanonicalize)
        if 'err_msg' in template:
            return template
    return template


def extract_from_reaction_smiles(reaction_smiles, _id=0, keep_atom_mapping=False, r_radius=1, p_radius=0, acc=10,
                                 precanonicalize=True):
    reaction = {}
    reaction['_id'] = _id
    reaction['reactants'] = reaction_smiles.split('>')[0]
    reaction['products'] = reaction_smiles.split('>')[-1]
    return extract_from_reaction(reaction, keep_atom_mapping=keep_atom_mapping, r_radius=r_radius, p_radius=p_radius,
                                 acc=acc, precanonicalize=precanonicalize)


def get_changed_atoms_from_rxn(rxn_smi):
    reactants = mols_from_smiles_list(replace_deuterated(rxn_smi.strip().split('>')[0]).split('.'))
    products = mols_from_smiles_list(replace_deuterated(rxn_smi.strip().split('>')[-1]).split('.'))

    # if rdkit cant understand molecule, return
    if None in reactants: return {'reaction': rxn_smi,
                                  'err_msg': 'rdkit cant understand reactants molecule'}
    if None in products: return {'reaction': rxn_smi,
                                 'err_msg': 'rdkit cant understand products molecule'}
...
def get_changed_atoms_from_rxn_smarts(rxn_smi):
    reactants = mols_from_smarts_list(replace_deuterated(rxn_smi.strip().split('>')[0]).split('.'))
    products = mols_from_smarts_list(replace_deuterated(rxn_smi.strip().split('>')[-1]).split('.'))
...
def atoms_are_different_nomap(atom1, atom2):
    '''Compares two RDKit atoms based on basic properties'''
    ...
    # Check bonds and nearest neighbor identity
    bonds1 = sorted([bond_to_label(bond, False) for bond in atom1.GetBonds()])
    bonds2 = sorted([bond_to_label(bond, False) for bond in atom2.GetBonds()])
    if bonds1 != bonds2: return True
...
def get_atom_feature(atom):
    feature = {}
    feature['AtomicNum'] = atom.GetAtomicNum()
    feature['TotalNumHs'] = atom.GetTotalNumHs()
    feature['FormalCharge'] = atom.GetFormalCharge()
    feature['Degree'] = atom.GetDegree()
    feature['NumRadicalElectrons'] = atom.GetNumRadicalElectrons()
    feature['Aromatic'] = atom.GetIsAromatic()
    bonds = sorted([bond_to_label(bond, False) for bond in atom.GetBonds()])
    feature['bonds'] = bonds
